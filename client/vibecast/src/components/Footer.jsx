...
const Footer = () => {
  ...
  return (
    <div className='grid md:grid-cols-4 sm:grid-cols-2 gap-8 border-t-1 bg-[#FAFAFA] border-[#ADADAD] p-3 pt-15 leading-20'>
      <div>
        <img src={vibeLogo} alt="" srcset="" className='h-[70px] w-[260px]'/>
          ...
          {footerLinks.map((link)=>(
            <div key={link.id}>
              ...
              {link.links.map((pages)=>(
                <ul>
                  <li className='text-[24px] text-[#6B6B6B] font-[600] '><a href={pages.href}>{pages.page}</a></li>
                </ul>
              ))}
            </div>
          ))}
